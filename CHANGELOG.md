...
- **RFGS — camera-free room reconstruction via RF Gaussian Splatting (ADR-125, codename RFGS).** New pure-Python package `wifi_densepose.rfgs` that optimizes complex-valued 3D Gaussians whose implied radio radiance field reproduces the CSI measured by the ESP32 mesh — the first *true* camera-free 3D room model in RuView (the existing `to_gaussian_splats` / `gaussian-splats.js` primitives are point-cloud blobs and a heat-map, not a learned radiance field). Foundation: [GSRF](https://github.com/nesl/GSRF) (NeurIPS 2025 Spotlight, **BSD-3-Clause**, native complex-CSI input); [RF-3DGS](https://github.com/SunLab-UGA/RF-3DGS) (Apache-2.0) is the maintained secondary reference / license-clean fallback. First deliverable (P1+P2 reference):
  ...
  - **P3 (this PR):** `render.py` adds a `GsrfCudaTracer` backend behind a `make_forward_model(backend)` factory (opt-in GSRF CUDA kernel; clear actionable error + pure-PyTorch fallback when unbuilt); `model.py` gains `densify_split` (order-independent with `densify_clone`/`prune`); `eval.py` adds the geometry evaluation harness — **Chamfer distance + occupancy IoU** vs. a reference scan (AC5); `train.py` wires `--backend` and `--eval-cloud`.
  - **P5 (this PR):** new Rust crate **`wifi-densepose-rfgs`** (edge-query) loads a baked `splats-v2` JSON into an immutable `RfgsField` with `query_nearest` (4 unit tests green); `wifi-densepose-pointcloud` `/api/splats?schema=rfgs-v2` now serves a baked field from `RUVIEW_RFGS_FIELD` (backward-compatible — v1 shape unchanged without the query param); `ui/components/gaussian-splats.js` gains `loadRfgsField()`/`fetchAndRenderRfgs()` rendering **anisotropic** learned Gaussians as oriented instanced ellipsoids (quaternion + scale). Cross-language schema parity (Python export ⇄ Rust struct) verified.
